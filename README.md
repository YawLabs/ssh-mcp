...
The pool caps at 100 connections by default, counting connections still being dialed. Set `SSH_MCP_MAX_POOL_SIZE=<n>` to raise it for fan-out workloads against many distinct hosts. `ssh_multi_exec` runs at most that many hosts at once and works through a larger fleet as slots free up, so raising the cap raises its parallelism. When the cap is reached, the pool evicts an idle entry to make room; if no idle entry can be evicted (every slot is in use or dialing), the call waits for one. A single-host tool waits up to its `timeout` (30s for the SFTP tools, which have none) and is next in line for the first slot that frees — so an `ssh_exec` arriving in the middle of a wide `ssh_multi_exec` runs between two of its hosts rather than being refused for the whole run — and reports `Connection pool is full` only if nothing freed in that time. `ssh_multi_exec` keeps one budget for the whole call: it gives up only after one `timeout` passes with no host of the call starting or finishing, at which point the host waiting and every host still queued report `Connection pool is full`; a fan-out that keeps moving never trips it, however long it runs. The pool is shared by every tool, so a slot held by a long `ssh_exec` or a concurrent `ssh_multi_exec` is waited out rather than counted as a failed host.
