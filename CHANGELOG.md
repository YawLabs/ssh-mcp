...
- A repeated `SIGINT`/`SIGTERM` now escalates to `SIGKILL` and exits `128+n`, so a wedged child no longer leaves the launcher hanging with no escape hatch. Registering a handler suppresses Node's default terminate-on-signal, and `child.killed` records only that `kill()` was *called* — never that the child is gone — so gating on it swallowed every signal after the first. Escalation is **time**-gated rather than count-gated: a supervisor routinely sends `SIGINT` then `SIGTERM` milliseconds apart, and a terminal Ctrl-C reaches the whole process group, so treating the second signal as impatience would `SIGKILL` the child mid-shutdown — skipping its `process.on("exit")` backstop and leaking any ssh-agent the server spawned. Inside a 2s window signals are forwarded as-is; only a child still alive after it is killed.
