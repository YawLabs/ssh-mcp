...
### Added
- Library API: `acquire()` now rejects a full pool with a `PoolFullError` carrying `code: "ERR_SSH_MCP_POOL_FULL"` (`POOL_FULL_ERROR_CODE`, matched by `isPoolFullError()`), and `ConnectionPool.waitForCapacity(timeoutMs)` parks a caller until a slot may be free, resolving `true` on a wake and `false` on timeout. `acquire()` and `withConnection()` still fail fast by default; pass `{ waitForCapacityMs }` (`AcquireOptions`) and they park on that primitive and retry on every capacity signal until a slot is won or the budget is spent, then reject with a `PoolFullError` whose message ends in `no slot freed up within <n>ms`. Every tool uses it, as described below.
...
- **`ssh_multi_exec` waits for pool capacity instead of failing the rest of its queue.** The pool is shared by every tool, so a slot held elsewhere — a long `ssh_exec`, a concurrent `ssh_multi_exec` — made a fan-out's acquire reject with `Connection pool is full`; that rejection was recorded as the host's result and the worker moved to the next host, which was rejected the same way, so one rejected worker failed every remaining host within microtasks (measured: cap 3 with one slot held elsewhere and 9 hosts failed 7; cap 4 with two concurrent 8-host calls failed the second call's 8 hosts). A capacity rejection is now treated as backpressure, not a host failure: the worker parks until the pool signals a slot may be free (a release, an entry closing or expiring, a failed dial, or `drain()`) and retries the same host. The wait is bounded by ONE budget for the whole call, the call's `timeout`, measured from its last progress — the last time any of its hosts was handed a slot or gave one back — so a fan-out that keeps moving never trips it however long it runs, while a call whose pool frees nothing ends after about one `timeout` with no progress: the host waiting at that point and every host still queued report `Connection pool is full` at once, without a dial. (A per-host budget on the serial worker queue made that starved call settle only after one `timeout` per worker-turn — cap 4, 40 hosts, 1s: 10s, every host reporting pool-full in turn. The trade is that slots freeing only after 2.5x `timeout` used to still complete the later hosts; now they do not.) Connect and exec errors are still recorded at once.
- **Single-host tools no longer starve while an `ssh_multi_exec` wider than the pool cap is running.** A fan-out's worker releases its slot and re-acquires it for the next host within one microtask drain, so a fail-fast `ssh_exec` (or any of the other eleven single-host tools) arriving from a later turn of the event loop found the pool full at every attempt for essentially the whole run (measured: cap 3, 9 hosts, 10ms dial, 20ms command — rejected at every probe from 5ms to 125ms of a ~135ms run). Every single-host tool now waits for a slot for as long as its own command may run (`timeout`; 30s for the SFTP tools, which have none) and is next in line: the pool wakes it on the very release that would otherwise have gone to the fan-out's next host, so it runs between two of that call's hosts. `Connection pool is full` is reported only once nothing has freed in that time.
