...
- The on-disk key in that branch is the first readable entry from `ssh -G`'s `identityfile` list. OpenSSH emits that list for every host even with no `IdentityFile` line, so it is the normal path; the hardcoded `id_ed25519`/`id_rsa`/`id_ecdsa` list is reached only when `ssh -G` fails (no ssh client). Only the FIRST existing key is offered — there is no walk of the whole list.
- When an agent is configured, an ENCRYPTED on-disk key is skipped (`isEncryptedKey`, ssh.ts): ssh2 parses `privateKey` eagerly and throws "no passphrase given", which would break the encrypted-key-on-disk / decrypted-copy-in-agent setup. With no agent the first existing key is loaded regardless. `agentSock` defaults to the Windows named pipe unconditionally (no service check), so on Windows the skip is always in force.
